...
class Tensor:
    ...
    def all(self, dim: Optional[int] = None) -> Tensor:
        ...
        if dim is None:
            dim = -1
        dim_int = self._ensure_tensor(dim)
        return All.apply(self, dim_int)
    ...
    def sum(self, dim: Optional[int] = None) -> Tensor:
        ...
        if dim is None:
            dim2 = self._ensure_tensor(-1)
            return Sum.apply(
                self.contiguous().view(int(operators.prod(self.shape))), dim2
            )
        else:
            dim2 = self._ensure_tensor(dim)
            return Sum.apply(self, dim2)
    ...
    def mean(self, dim: Optional[int] = None) -> Tensor:
        ...
        return self.sum(dim) / total_elements

    def permute(self, *order: int) -> Tensor:
        """Permute the dimensions of this tensor."""
        order_tensors = tuple(self._ensure_tensor(i) for i in order)
        order_tensor = Tensor.make(
            [int(i.item()) for i in order_tensors],
            (len(order_tensors),),
            backend=self.backend,
        )
